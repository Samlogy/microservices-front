...
//                "  -Dsonar.projectKey=angular-api \\\n" +
//                "  -Dsonar.projectName='angular-api' \\\n" +
...
// build app
  ...
  stage("build image") {
    sh "sudo docker build -t angular-ui ."
  }
  ...
  stage("push docker image") {
    withCredentials([usernamePassword(credentialsId: 'sammmmmm', usernameVariable: 'username',
      ...
      sh "sudo docker tag angular-ui sammmmmm/angular-ui:$branchName"
      sh "sudo docker push sammmmmm/angular-ui:$branchName"
      sh "sudo docker rmi sammmmmm/angular-ui:$branchName"
      sh "sudo docker rmi angular-ui"
